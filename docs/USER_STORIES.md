...
- Next.js 16 client uses Anchor/SDK to construct transactions.
- Deposit is routed through the HUSH Vault program and state is recorded in MagicBlock PER.
- The landing page features an immersive, responsive hero section that reacts to wallet connection.
...
- Rebalance events are visualised in the dashboard as "energy flows" or fluid movements.
...
**Acceptance Criteria:**
- Next.js frontend interacts directly with the MagicBlock RPC to submit the grant advisory.
- The `GrantRequest` state uses `INIT_SPACE` for accurate memory allocation.
- Grant status is privately queryable via the MagicBlock RPC using the donor's viewing key.

## HUSH-004: The Auditor — Compliance & Selective Disclosure
...
**Acceptance Criteria:**
- The system queries the MagicBlock PER directly using the viewing key hash.
- Generates a structured JSON tax receipt with a cryptographic proof of inclusion.
- Verification logic is covered by Anchor integration tests.
- The UI displays the tax receipt in a polished, readable panel with smooth entrances.
