...
pub struct AdviseGrant<'info> {
    ...
    #[account(
        ...
        seeds = [b"grant_request", daf_account.key().as_ref(), charity_wallet.as_ref(), &daf_account.total_granted.to_le_bytes()],
        bump
    )]
...
pub fn handler(ctx: Context<AdviseGrant>, charity_wallet: Pubkey, amount: u64, tax_year: u16) -> Result<()> {
    let daf_account = &mut ctx.accounts.daf_account;
    
    // Check balance
    require!(daf_account.balance_usdc >= amount, ErrorCode::InsufficientBalance);

    // Deduct balance
    daf_account.balance_usdc = daf_account.balance_usdc.checked_sub(amount).ok_or(ErrorCode::MathUnderflow)?;
    daf_account.total_granted = daf_account.total_granted.checked_add(1).ok_or(ErrorCode::MathOverflow)?;
